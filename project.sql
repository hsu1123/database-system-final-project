...
SET @crank=0;
SELECT @crank:=@crank+1, C.course_name, K.F
FROM COURSE_INFO AS C, (
	SELECT R.course_id, AVG(feedback_rank) AS F
    FROM (SELECT GRADES.* FROM GRADES, STUDENT WHERE GRADES.student_id = STUDENT.student_id AND STUDENT.student_status = '在學' AND GRADES.select_result = '中選') AS R
...
FROM (SELECT GRADES.* FROM GRADES, STUDENT WHERE GRADES.student_id = STUDENT.student_id AND STUDENT.student_status = '在學' AND GRADES.select_result = '中選') AS R, COURSE_INFO AS C
GROUP BY course_id
